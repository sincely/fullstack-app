  ...
  <TransitionGroup tag="div" name="setting-list" class="flex-col-stretch gap-12px">
    ...
    <SettingItem v-if="themeStore.page.animate" key="1-2" :label="'页面切换动画类型'">
      <a-select v-model:value="themeStore.page.animateMode" class="w-120px">
        <a-select-option v-for="option in themePageAnimationModeOptions" :key="option.value" :value="option.value">
          {{ option.label }}
        </a-select-option>
      </a-select>
    </SettingItem>
    ...
    <SettingItem key="3" :label="'头部高度'">
      <a-input-number v-model:value="themeStore.header.height" class="w-120px" />
    </SettingItem>
    ...
    <SettingItem v-if="themeStore.tab.visible" key="5-2" :label="'标签栏高度'">
      <a-input-number v-model:value="themeStore.tab.height" class="w-120px" />
    </SettingItem>
    <SettingItem v-if="themeStore.tab.visible" key="5-3" :label="'标签栏风格'">
      <a-select v-model:value="themeStore.tab.mode" class="w-120px">
        <a-select-option v-for="option in themeTabModeOptions" :key="option.value" :value="option.value">
          {{ option.label }}
        </a-select-option>
      </a-select>
    </SettingItem>
    <SettingItem v-if="layoutMode === 'vertical'" key="6-1" :label="'侧边栏宽度'">
      <a-input-number v-model:value="themeStore.sider.width" class="w-120px" />
    </SettingItem>
    <SettingItem v-if="layoutMode === 'vertical'" key="6-2" :label="'侧边栏折叠宽度'">
      <a-input-number v-model:value="themeStore.sider.collapsedWidth" class="w-120px" />
    </SettingItem>
    <SettingItem v-if="isMixLayoutMode" key="6-3" :label="'混合布局侧边栏宽度'">
      <a-input-number v-model:value="themeStore.sider.mixWidth" class="w-120px" />
    </SettingItem>
    <SettingItem v-if="isMixLayoutMode" key="6-4" :label="'混合布局侧边栏折叠宽度'">
      <a-input-number v-model:value="themeStore.sider.mixCollapsedWidth" class="w-120px" />
    </SettingItem>
    <SettingItem v-if="layoutMode === 'vertical-mix'" key="6-5" :label="'混合布局子菜单宽度'">
      <a-input-number v-model:value="themeStore.sider.mixChildMenuWidth" class="w-120px" />
    </SettingItem>
    ...
    <SettingItem v-if="themeStore.footer.visible" key="7-2" :label="'底部高度'">
      <a-input-number v-model:value="themeStore.footer.height" class="w-120px" />
    </SettingItem>
